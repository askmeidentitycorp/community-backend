...
class AuthController {
  ...
  static async ensureGeneralForUser(user, chimeDetails = {}) {
    try {
      ...
      logger.warn("Auth: ensureGeneralForUser failed (continuing)", {
        error: err?.message,
      });
    }
  }
  ...
  async exchangeToken(req, res, next) {
    try {
      ...
      const { accessToken, refreshToken, sessionId } =
        await tokenService.createSession(user._id.toString(), deviceInfo);
  ...
  async auth0CodeExchange(req, res, next) {
    try {
      ...
      if (!id_token) {
        throw new AppError(
          "Missing id_token from Auth0",
          500,
          "AUTH0_EXCHANGE_FAILED"
        );
      }
      ...
      if (!user) {
        ...
        if (tenantId) {
          const result = await auth0Service.addUserToTenant(
            tenantId.toString(),
            user._id.toString(),
            "member"
          );
          if (!result.success) {
            logger.error("Auth0: failed to add user to tenant", {
              userId: user._id.toString(),
            ...
            throw new AppError(
              result.message,
              500,
              "TENANT_USER_ADDITION_FAILED"
            );
          }
          tenantUserLinkId = result.data._id;
          logger.info("Auth0: user added to tenant", {
            userId: user._id.toString(),
          ...
          logger.warn(
            "Auth0: tenantId not provided, user created without tenant link",
            { userId: user._id.toString() }
          );
        }
      } else {
        ...
        await user.save();
        logger.info("Auth0: user login (code-exchange)", {
          userId: user._id.toString(),
          email,
        });
        ...
        if (tenantId) {
          ...
          if (tenantUserLink) {
            ...
            logger.info("Auth0: found existing tenant user link", {
              tenantUserLinkId,
            });
          } else {
            logger.warn(
              "Auth0: no tenant user link found for existing user creating new tenantUserLink",
              { userId: user._id.toString(), tenantId }
            );
            const result = await auth0Service.addUserToTenant(
              tenantId.toString(),
              user._id.toString(),
          ...
          logger.warn("Auth0: tenantId not provided for existing user login", {
            userId: user._id.toString(),
          });
        }
      }
      ...
      const { accessToken, refreshToken, sessionId } =
        await tokenService.createSession(
          ...
          {
            CHIME_APP_INSTANCE_ARN: tenantDetail?.ChimeAppInstanceArn ?? "",
            CHIME_BEARER: tenantDetail?.ChimeBerear ?? "",
            CHIME_BACKEND_ADMIN_ROLE_ARN:
              tenantDetail?.ChimeBackendAdminRoleArn ?? "",
          }
        );
  ...
  async refreshAuth0IdToken(req, res, next) {
    try {
      ...
      const session = await Session.findOne({ userId: user._id }).sort({
        createdAt: -1,
      });
      if (!session || !session.auth0RefreshToken) {
        throw new AppError(
          "No Auth0 refresh token found",
          400,
          "NO_REFRESH_TOKEN"
        );
      }
      ...
      logger.error("Auth: refreshAuth0IdToken error", {
        error: error?.message,
      });
      next(error);
    }
  ...
  async validateToken(req, res, next) {
    try {
      ...
      res.status(200).json({
        ...
        expires_in: parseInt(
          process.env.JWT_ACCESS_TOKEN_LIFETIME || "3600",
          10
        ),
      });
    } catch (error) {
  ...
  async getSessions(req, res, next) {
    try {
      ...
      logger.info("Auth: getSessions success", {
  ...
  async revokeAllSessions(req, res, next) {
    try {
      ...
      const success = await tokenService.revokeAllSessions(
        req.auth.userId,
        req.auth.sessionId
      );

      if (success) {
        logger.info("Auth: revokeAllSessions success", {
          userId: req.auth.userId,
        });
    ...
    try {
      logger.info("Auth0: login initiate", {
        ...
        auth0: {
          ...
          issuer:
            auth0Config.issuer || `https://${auth0Config.domain || "missing"}/`,
  ...
  async auth0Callback(req, res, next) {
    try {
      ...
      if (
        stored.ip !== req.ip ||
        stored.userAgent !== req.headers["user-agent"]
      ) {
      ...
      fe.searchParams.set(
        "expires_in",
        String(parseInt(process.env.JWT_ACCESS_TOKEN_LIFETIME || "3600", 10))
      );
  ...
  async onboardTenant(req, res, next) {
    try {
      ...
      const { tenantName, slug, domain, email, phoneNumber, status, password } =
        req.body;
      const deviceInfo = {
        userAgent: req.headers["user-agent"],
        ip: req.ip,
      ...
      const result = await auth0Service.onboardTenant(
        { tenantName, slug, domain, email, phoneNumber, status, password },
        deviceInfo
      );

      if (!result.success) {
        logger.error("Auth: onboardTenant failed", {
          tenantDomain: domain,
          reason: result.message,
        });
  ...
  getTenantOrganizations = async (req, res, next) => {
    try {
      ...
      logger.info("Auth: getTenantOrganizations success", {
        tenantId,
        organizationCount: result.data.length,
      });
    } catch (error) {
      logger.error("Auth: getTenantOrganizations error", {
        error: error?.message,
      });
      next(error);
    }
