...
class TokenService {
  ...
  generateAccessToken(payload) {
    ...
    return jwt.sign({ ...payload, type: "access", version: 1 }, secret, {
      expiresIn: ttl,
    });
  }
  ...
  generateRefreshToken(payload) {
    ...
    return jwt.sign({ ...payload, type: "refresh", version: 1 }, secret, {
      expiresIn: ttl,
    });
  }
  ...
  async createSession(
    ...
    auth0Token = null,
    auth0Subject = null,
    auth0RefreshToken = null,
    tenantId = null,
    tenantUserLinkId = null,
    chimeInstaceData = null
  ) {
    try {
      logger.info("TokenService: createSession start", {
        userId,
        tenantId,
        tenantUserLinkId,
      });
      ...
      const refreshTtl = parseInt(
        process.env.JWT_REFRESH_TOKEN_LIFETIME || "604800",
        10
      );
      ...
      await session.save();
      logger.info("TokenService: session saved", {
      ...
      logger.error("TokenService: failed to create session", {
        error: error?.message,
      });
  ...
  async revokeAllSessions(userId, currentSessionId) {
    try {
      logger.info("TokenService: revokeAllSessions", {
        userId,
        currentSessionId,
      });
  ...
  async refreshAccessToken(refreshToken) {
    try {
      ...
      logger.info("TokenService: session updated with new access token", {
        sessionId: session._id.toString(),
      });
